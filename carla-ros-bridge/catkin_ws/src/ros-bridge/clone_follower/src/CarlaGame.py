...
class CarlaGame():
    def __init__(self):
        ...
        self.a_min = -6.0 #-5.51127117000001       #m2/s
        ...
        self.sub_leading_vehicle_position = rospy.Subscriber("/carla/hero3/odometry",Odometry,self.leading_vehicle_position_callback,queue_size = 10)
        self.sub_leading_vehicle_velocity = rospy.Subscriber("/carla/hero3/vehicle_status",CarlaEgoVehicleStatus,self.leading_vehicle_velocity_callback,queue_size = 10)
        self.sub_following_vehicle_acceleration_and_velocity = rospy.Subscriber("/carla/hero4/vehicle_status",CarlaEgoVehicleStatus,self.following_vehicle_acceleration_and_velocity_callback,queue_size = 10)
        ...
        self.sub_collision = rospy.Subscriber("/carla/hero4/collision", CarlaCollisionEvent, self.following_vehicle_collision_callback,queue_size = 10)
        self.pub = rospy.Publisher('/carla/hero4/vehicle_control_cmd',CarlaEgoVehicleControl,queue_size = 10)
        self.reset()
    ...
    def from_velocity_to_gas(self):
        ...
        self.output_throttle = math.exp(self.FC_output3_layer[0,0])/(1.0 + math.exp(self.FC_output3_layer[0,0]))
        self.output_brake =  math.exp(self.FC_output3_layer[1,0])/(1.0 + math.exp(self.FC_output3_layer[1,0]))
    ...
    #return: states               
        self.lead_pub = rospy.Publisher('/carla/hero3/vehicle_control_cmd',CarlaEgoVehicleControl,queue_size = 10)
        self.follow_pub = rospy.Publisher('/carla/hero4/vehicle_control_cmd',CarlaEgoVehicleControl,queue_size = 10)
        self.Follow_control = CarlaEgoVehicleControl()        
        self.Lead_control = CarlaEgoVehicleControl()        
    ...
    def getRelativPosition(self):        
    	...
    	s = lead_position - follow_position  -4.4 
    	if s > self.s_max:
            s = self.s_max
    	return s
    ...
    #return: state, reward, done
        ...
        accleration = min(action[0,0]*6.0,5.0)
        ...
        if len(self.collision_list) != 0 or (self.Lead_Veh_Position_x -self.Follow_Veh_Position_x-4.4)  <= 0:
            done = True
        else:
            done = False
