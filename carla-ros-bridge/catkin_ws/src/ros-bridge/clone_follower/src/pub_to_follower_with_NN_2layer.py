...
class CarlaAckermanControl(object):
    def __init__(self):
        ...
        self.sub_vehicle_velocity = rospy.Subscriber("/carla/hero4/vehicle_status",CarlaEgoVehicleStatus,self.follower_velocity_callback,queue_size = 1)        
    ...
    def publish_to_vehicle(self):
        self.pub = rospy.Publisher('/carla/hero4/vehicle_control_cmd',CarlaEgoVehicleControl,queue_size = 1)
        ...
        while not rospy.is_shutdown():
            for i in range(episode_count):
                ...
                for i in range(self.start_index,self.start_index + self.max_steps):
                    print("soll follower velocity",self.vf[i])
                    self.NN_INPUT[0,0] = self.vf[i]
...
def main():
    rospy.init_node('follower_clone_follower',anonymous=True)
